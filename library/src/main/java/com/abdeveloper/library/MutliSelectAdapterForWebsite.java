...
class MutliSelectAdapterForWebsite extends RecyclerView.Adapter<MutliSelectAdapterForWebsite.MultiSelectDialogViewHolder> {

    private ArrayList<MultiSelectModelForWebsite> mDataSet = new ArrayList<>();
    ...
    MutliSelectAdapterForWebsite(ArrayList<MultiSelectModelForWebsite> dataSet, Context context) {
    ...
    public void onBindViewHolder(final MultiSelectDialogViewHolder holder, int position) {
        ...
        if (smm == 0) {
            ...
            if (mDataSet.get(position).getSelected()) {
                if (!MultiSelectDialogForWebsites.selectedIdsForCallback.contains(mDataSet.get(position).getId())) {
                    MultiSelectDialogForWebsites.selectedIdsForCallback.add(mDataSet.get(position).getId());
                }
            }
        }
        ...
        holder.main_container.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!holder.dialog_item_checkbox.isChecked()) {
                    MultiSelectDialogForWebsites.selectedIdsForCallback.add(mDataSet.get(holder.getAdapterPosition()).getId());
                    holder.dialog_item_checkbox.setChecked(true);
    ...
    private void removeFromSelection(String id) {
        for (int i = 0; i < MultiSelectDialogForWebsites.selectedIdsForCallback.size(); i++) {
            if (id.equals(MultiSelectDialogForWebsites.selectedIdsForCallback.get(i))) {
                MultiSelectDialogForWebsites.selectedIdsForCallback.remove(i);
            }
        }
    }
    ...
    void setData(ArrayList<MultiSelectModelForWebsite> data, String query, MutliSelectAdapterForWebsite mutliSelectAdapter) {
        this.mDataSet = data;
    ...
    public void selectALl(boolean isSelectedAll) {
        ...
        if (isSelectedAll) {
            for (MultiSelectModelForWebsite i : mDataSet) {
                MultiSelectDialogForWebsites.selectedIdsForCallback.add(i.getId());
            }
        }
